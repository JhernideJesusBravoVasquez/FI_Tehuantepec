...
class _VerActividadesEstudianteState extends State<Avance> {
    ...
    try {
      ...
      if (participacionesSnapshot.docs.isEmpty) {
        ...
        for (var doc in participacionesSnapshot.docs) {
          String actividadId = doc['idActividad'];
  ...
  void _aplicarFiltro() {
    ...
    if (sumaValoresTotal >= LIMITE_TOTAL) {
      mensajeAdvertencia = '¡Se ha alcanzado el límite total de $LIMITE_TOTAL horas!';
    } else if (_categoriaSeleccionada != 'Todas' && sumaValoresCategoria >= limiteCategoria) {
      mensajeAdvertencia =
          '¡Se ha alcanzado el límite de $limiteCategoria horas en la categoría $_categoriaSeleccionada!';
    }
