...
class _CrearActividadPageState extends State<CrearActividad> {
  ...
  // Determinar la inicial de la categoría
  String inicialCategoria;
  switch (_categoriaSeleccionada) {
    ...
    case 'academica':
      ...
      break;
    default:
      inicialCategoria = ''; // Manejar posibles casos no esperados
  }

  // Construir el prefijo del idActividad basado en la fecha
  String idActividadBase =
      '${_diaSeleccionado!.padLeft(2, '0')}${_mesSeleccionado!.padLeft(2, '0')}${_anioSeleccionado!}-$inicialCategoria';

  try {
    // Consultar cuántas actividades ya existen con este prefijo
    QuerySnapshot actividadesExistentes = await FirebaseFirestore.instance
        .collection('activities')
        .where('idActividad', isGreaterThanOrEqualTo: idActividadBase)
        .where('idActividad', isLessThan: '$idActividadBase~') // ~ asegura que solo se incluyan actividades con este prefijo
        .get();

    int numeroConsecutivo = actividadesExistentes.docs.length + 1;

    // Crear el idActividad final con el número consecutivo
    String idActividad = '$idActividadBase$numeroConsecutivo';
