...
class ModificarActividad extends StatefulWidget {
  ...
  ModificarActividad({required this.actividadId, required this.actividadData, required this.onDelete});
...
class _ModificarActividadPageState extends State<ModificarActividad> {
  ...
  // Crear el timestamp para la nueva fecha y hora
  Timestamp fechaHoraTimestamp = _crearTimestamp();

  // Generar el nuevo ID basado en la nueva fecha y categoría
  String nuevaFecha = '${_diaSeleccionado!.padLeft(2, '0')}${_mesSeleccionado!.padLeft(2, '0')}${_anioSeleccionado!}';
  String inicialCategoria = _categoriaSeleccionada!.substring(0, 1).toUpperCase();
  String nuevoIdBase = '$nuevaFecha-$inicialCategoria';

  // Incrementar el número si hay actividades con el mismo ID base
  String nuevoId = nuevoIdBase;
  int contador = 1;
  ...
  while (idExistente) {
    ...
    if (docSnapshot.exists && nuevoId != widget.actividadId) {
      contador++;
      nuevoId = '$nuevoIdBase$contador';
    } else {
      idExistente = false;
    }
  }
  ...
  try {
    // Si el ID ha cambiado, eliminar el documento con el ID antiguo
    if (nuevoId != widget.actividadId) {
      await FirebaseFirestore.instance.collection('activities').doc(widget.actividadId).delete();
    }

    // Crear/actualizar el documento con el nuevo ID
    await FirebaseFirestore.instance.collection('activities').doc(nuevoId).set({
      ...
      'idActividad': nuevoId, // Guardar el nuevo ID en el documento
    });
  ...
  Future<void> _eliminarActividad() async {
    try {
      await FirebaseFirestore.instance.collection('activities').doc(widget.actividadId).delete();
      ...
      widget.onDelete();
      Navigator.pop(context); // Regresar a la página anterior después de eliminar
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text('¿Estás seguro de que deseas eliminar esta actividad? Esta acción no se puede deshacer.'),
          actions: [
            TextButton(
              child: Text('Cancelar'),
              onPressed: () {
